...
- **Wiring up data, handlers, routing for pages** — Figma can't tell us how the app actually works. Producing confident-but-wrong code in this area is worse than producing nothing. Page output is intentionally a UI shell that defers integration to the dev (or to the existing `/build` skill).
